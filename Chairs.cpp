...
int dp[1001][501][2];
int n, k;
int solve(int i, int left, bool b = 0){
    if (i > n)  if (left == 0)  return 1;   else return 0;
    if (i == n){
        if (left > 1)  return 0;
        if (left == 1) if (b)  return 0; else return 1;
        return 1;
    }
    if (left == 0)  return 1;
    if (dp[i][left][b] != -1)  return dp[i][left][b];
    // Take this chair
    int ans = solve(i+2, left-1, i == 1 ? 1 : b);
    // Leave this chair
    ans += solve(i+1, left, b);

    return dp[i][left][b] = ans % mod;
}
